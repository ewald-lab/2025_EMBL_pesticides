...
def main():
    ...
    #standardized_pesticides = StandardizeMolecule(input=pesticides, augment=True, num_cpu=4).run()
    #standardized_pesticides.to_csv("../data/standardized_pesticides.csv", index=False)

    standardized_drugs = StandardizeMolecule(input=drugs, augment=True, num_cpu=4).run()
    standardized_drugs.to_csv("../data/standardized_drugs.csv", index=False)

    standardized_refchemdb = StandardizeMolecule(input=refchemdb, augment=True, num_cpu=4).run()
